    ...
    <div className="flex flex-col items-center justify-center p-10 bg-white shadow-xl rounded-2xl border border-gray-100 gap-6 min-w-md">
      ...
      <div className="flex flex-col items-center">
        {status === "ACTIVE" && (
          <>
            <div
              className={`px-4 py-1 rounded-full text-sm font-mono font-bold transition-colors ${timeLeft < 20 ? "bg-red-100 text-red-600 animate-pulse" : "bg-indigo-100 text-indigo-600"}`}
            >
        ...
        <p
          className={cn(
            "px-4 py-1 rounded-full font-mono font-bold text-sm bg-indigo-100 text-indigo-500",
            status === "EXPIRED" && "bg-red-100 text-red-500",
            status === "ACTIVE" && "hidden",
          )}
        >
          {status === "EXPIRED"
            ? "Session timed out. Click above to try again."
            : "Click above Button to Generate QR Code"}
        </p>
      </div>
    </div>
  );
