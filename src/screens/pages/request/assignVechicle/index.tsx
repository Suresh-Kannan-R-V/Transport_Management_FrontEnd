...
export const VehicleAssignmentPopup = ({
  ...
  const fetchVehicles = async () => {
    try {
      const response = await fetch(
        `${FILE_BASE_URL}/api/vehicles/get-all?limit=100&status=active`,
        {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (source.droppableId === "guest-pool") {
      guestToMove = unassignedGuests.find((g) => g.id.toString() === draggableId);
      setUnassignedGuests((prev) => prev.filter((g) => g.id.toString() !== draggableId));
    } else {
      const sourceCard = vehicles.find((v) => v.id === source.droppableId);
      guestToMove = sourceCard?.assignedGuests.find((g) => g.id.toString() === draggableId);
      setVehicles((prev) =>
        prev.map((v) =>
          v.id === source.droppableId
            ? { ...v, assignedGuests: v.assignedGuests.filter((g) => g.id.toString() !== draggableId) }
            : v
        )
      );
    }
          ...
          <div
            ...
            className={cn(
              ...
              snapshot.isDraggingOver
                ...
                : "border-transparent",
              // Apply red border/background if over capacity
            ...
            <div className="flex items-center justify-between mb-4 gap-2">
              ...
              <Button
                ...
                onPress={() => {
                  ...
                  setVehicles((prev) => prev.filter((v) => v.id !== card.id));
                }}
              >
                <X size={16} />
              </Button>
            ...
            <div className="space-y-2 flex-1">
                ...
                >
                  {(p) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => returnGuestToPool(card.id, guest.id)}
              ...
              {card.assignedGuests.length === 0 && (
                <div className="h-24 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 rounded-2xl text-slate-300 uppercase text-[10px] font-bold">
                  <Users size={20} className="mb-1" /> Drop Guests Here
                </div>
              )}
            </div>
          </div>
