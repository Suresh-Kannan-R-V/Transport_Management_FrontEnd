...
export const Dashboard = () => {
  // Replace this with your actual user data/state
  ...
  return (
    <div className="w-full h-full animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 font-bold text-[11px] uppercase tracking-widest mt-1">
              TMS Logistics Monitor • Operational Status: <span className="text-emerald-500">Optimal</span>
            </p>
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-5 mb-8">
        ...
        <StatCard title="On Duty" value="24" icon={<ShieldAlert />} color="emerald" />
        <StatCard title="Conflicts" value="02" icon={<AlertTriangle />} color="rose" pulse />
        <StatCard title="Emergency" value="01" icon={<Flame />} color="orange" pulse />
      </section>
        ...
        <div className="col-span-12 xl:col-span-8 space-y-8">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-50 text-left">
                  <TableRow id="M-9021" vehicle="Toyota Hiace" driver="John Wick" time="14:30" />
                  <TableRow id="M-8842" vehicle="Honda Civic" driver="Sarah J." time="16:00" />
                  <TableRow id="M-7721" vehicle="Ford Transit" driver="Mike Ross" time="17:15" />
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <RequestCard id="REQ-101" faculty="Dr. Aris" date="Oct 12" pass="04" />
              <RequestCard id="REQ-102" faculty="Prof. Snape" date="Oct 14" pass="01" />
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 xl:col-span-4">
          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 h-full">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Utilization</h3>
                <p className="text-xs text-slate-400 font-medium mt-1">Weekly system load efficiency</p>
              </div>
                ...
                <BarChart data={utilizationData}>
                  ...
                  <Bar dataKey="value" radius={[8, 8, 8, 8]} barSize={22}>
                    {utilizationData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.value > 80 ? '#6366f1' : '#cbd5e1'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="mt-10 p-6 bg-indigo-50 rounded-[2rem] flex justify-between items-center group cursor-pointer hover:bg-indigo-100 transition-all border border-indigo-100">
              <div>
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] leading-none mb-2">Weekly Peak</p>
                <p className="text-2xl font-black text-indigo-900">Friday (90%)</p>
              </div>
    ...
    <div className="bg-white p-6 rounded-[2.25rem] border border-slate-100 shadow-sm flex flex-col gap-4 relative overflow-hidden hover:border-indigo-300 hover:shadow-xl hover:shadow-indigo-500/5 transition-all group">
      {pulse && (
        <span className="absolute top-6 right-6 flex h-2.5 w-2.5">
          <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${themes[color].split(' ')[1].replace('text', 'bg')}`}></span>
          <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${themes[color].split(' ')[1].replace('text', 'bg')}`}></span>
        </span>
      )}
      <div className={cn("w-12 h-12 rounded-[1.25rem] flex items-center justify-center border group-hover:scale-110 transition-transform duration-300", themes[color])}>
        {React.cloneElement(icon, { size: 24 })}
      </div>
      <div>
        <p className="text-3xl font-black text-slate-800 leading-none tracking-tight">{value}</p>
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mt-3">{title}</p>
      </div>
    </div>
  );
};

const TableRow = ({ id, vehicle, driver, time }: any) => (
  <tr className="group hover:bg-slate-50/80 transition-colors">
    ...
    <td className="px-8 py-6 text-left">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-sm font-bold text-slate-800 leading-tight">{vehicle}</p>
          <p className="text-[11px] text-slate-400 font-black uppercase tracking-wider mt-1">{driver}</p>
        </div>
      </div>
    </td>
...
const RequestCard = ({ id, faculty, date, pass }: any) => (
  <div className="border border-slate-100 bg-slate-50/40 p-6 rounded-[2.25rem] flex justify-between items-center group hover:bg-white hover:shadow-2xl hover:shadow-indigo-500/8 transition-all cursor-pointer">
    <div className="flex items-center gap-5">
      ...
      <div>
        ...
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1.5">{date} • {pass} Passengers</p>
      </div>
    </div>
